                ...
                if Character.Humanoid.Health > 0 and (esp.settings.max_distance == 0 or (Character.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < esp.settings.max_distance) then
                    ...
                    if onScreen and esp.settings.enabled then
                        ...
                        if esp.settings.drawings.box.enabled then
                            ...
                            v.box.Color = (esp.settings.target_settings.enabled and esp.settings.target_settings.target == i and esp.settings.target_settings.color) or (esp.settings.friend_settings.enabled and i:IsFriendsWith(game.Players.LocalPlayer.UserId) and esp.settings.friend_settings.color) or esp.settings.drawings.box.color
                            v.box.Visible = true
                        else
